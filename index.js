...
function fuga(resposta) {
  if (resposta === "P") {
    ...
    let resposta = prompt(
      `Boa decisão! A mais segura possível, tendo em vista a situação. Dorsa desenvolveu ${inventario[0]}, item que agora está armazenado na sua mochila de características. Dorsa pode fugir para a Inglaterra ('I') ou pode ir para Síria ('S'), qual você prefere?`
    );
...
function conversaNPC(resposta) {
  ...
  if (resposta == 1) {
    ...
    if (dialogo == 1) {
      alert(
        `Samira: "Você só está esquecendo de uma coisa, a capitã deste time sou eu, eu que decido quem entra e quem sai. Além disso, NUNCA mais ouse falar que comprei a arbitragem, se isso sair daqui eu acabo com você, e aí vai terque dizer tchau pro Badminton.`
      );
...
function repercucaoDiscussao(dialogo) {
